...
export function GoogleLoginButton({ intent }: GoogleLoginButtonProps) {
  ...
  const handleLoginSuccess = async (tokenResponse: any) => {
    ...
    try {
      const response: any = await fetchApi("/auth/google", {
        method: "POST",
        body: JSON.stringify({
          token: tokenResponse.access_token, // Gửi Access Token
          intent,
        }),
      });
  ...
  const handleButtonClick = () => {
    if (isLoading) return;
    setIsExpanding(true);
    // Kích hoạt login ngay khi bắt đầu animation
    setTimeout(() => {
        login();
    }, 100);
  };
    ...
    <div className="flex flex-col items-center w-full">
      ...
      <div className="w-full flex justify-center relative h-12">
        ...
        >
          ...
          {/* Hiệu ứng loading nếu cần */}
          {isLoading && (
            <motion.div 
                layoutId="loader"
                className="absolute inset-0 bg-white/20 backdrop-blur-[1px] flex items-center justify-center"
            >
