...
export function StudyHubWidget({ docId }: StudyHubWidgetProps) {
  ...
  return (
    <div className="mt-8 relative group">
      ...
      <div className="relative rounded-[2rem] border border-border bg-card/80 backdrop-blur-3xl overflow-hidden shadow-md">
        {/* Header toggle */}
        <button
          onClick={() => setOpen((v) => !v)}
          className="w-full flex items-center justify-between px-6 py-[18px] hover:bg-white/[0.02] transition-all duration-300"
        >
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col items-start pl-1.5 pr-1 py-1">
              <span className="mb-1 text-[11px] font-black uppercase leading-tight tracking-[0.2em] text-muted-foreground/60">Neural Core</span>
              <span className="text-[14px] font-black leading-tight tracking-tight text-foreground transition-colors group-hover:text-primary">Study Hub</span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <ChevronRight
              size={14}
              className={cn("text-muted-foreground/50 transition-transform duration-500 ease-out", open && "rotate-90")}
            />
          </div>
        </button>
          ...
          <div className="px-6 pb-6 space-y-5 animate-in fade-in slide-in-from-top-4 duration-500">
            ...
            {isLoading ? (
              <div className="flex flex-col gap-3 py-4 items-center justify-center">
                ...
                <span className="text-[11px] font-bold text-muted-foreground/60 uppercase tracking-widest">Đang đồng bộ dữ liệu...</span>
              </div>
            ) : mastery ? (
              <div className="space-y-4">
                <div className="flex items-end justify-between">
                  <div>
                    <p className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-widest mb-1">Cấp độ nắm vững</p>
                    <h4 className={cn("text-lg font-black tracking-tight", level.color)}>{level.label}</h4>
                  </div>
                  <div className="text-right">
                    <span className="text-2xl font-black text-foreground leading-none">{Math.round(masteryScore)}</span>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 rounded-2xl bg-muted/20 border border-border/50 hover:border-primary/20 transition-colors">
                    <p className="text-[9px] font-black text-muted-foreground/60 uppercase tracking-widest mb-1">🃏 Thẻ Nhớ</p>
                    <div className="flex items-center gap-2">
                      <p className="text-base font-black text-foreground">{Math.round(flashcardScore)}%</p>
                  ...
                  <div className="p-3 rounded-2xl bg-muted/20 border border-border/50 hover:border-amber-500/20 transition-colors">
                    <p className="text-[9px] font-black text-muted-foreground/60 uppercase tracking-widest mb-1">🏆 Kiểm Tra</p>
                    <div className="flex items-center gap-2">
                      <p className="text-base font-black text-foreground">{Math.round(quizScore)}%</p>
                      <div className="grow h-1 bg-muted rounded-full overflow-hidden">
              ...
              <div className="py-4 text-center">
                <p className="text-[12px] text-muted-foreground font-medium italic mb-2">Chưa có tiến độ học tập 📚</p>
                <p className="text-[10px] text-muted-foreground/60 leading-relaxed px-4">Hãy bắt đầu tạo flashcards hoặc quiz để AI đo lường mức độ thấu hiểu tài liệu của bạn.</p>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 gap-2.5 pt-2">
              <button
                ...
                className="group/btn relative w-full flex items-center gap-3 px-5 py-3.5 rounded-[1.25rem] bg-muted/30 border border-border text-muted-foreground hover:text-foreground hover:border-primary/30 transition-all duration-300"
              >
                ...
                <div className="flex flex-col items-start">
                  ...
                  <span className="text-[9px] text-muted-foreground/60 font-bold uppercase tracking-widest group-hover/btn:text-primary transition-colors">Flashcards AI</span>
                </div>
              ...
              <button
                ...
                className="group/btn relative w-full flex items-center gap-3 px-5 py-3.5 rounded-[1.25rem] bg-muted/30 border border-border text-muted-foreground hover:text-foreground hover:border-amber-500/30 transition-all duration-300"
              >
                ...
                <div className="flex flex-col items-start">
                  ...
                  <span className="text-[9px] text-muted-foreground/60 font-bold uppercase tracking-widest group-hover/btn:text-amber-500 transition-colors">Quiz AI Generation</span>
                </div>
