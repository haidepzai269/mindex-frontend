      ...
      <DialogContent className="sm:max-w-[450px] bg-card border-border text-foreground">
        ...
        {step === 1 && (
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="code" className="text-sm font-medium text-muted-foreground">Mã mời (8 ký tự)</Label>
              <Input
                id="code"
                placeholder="Ví dụ: ABCXYZ"
                maxLength={8}
                className="bg-muted/40 border-border focus:border-primary/30 text-foreground placeholder:text-muted-foreground/40 h-14 text-center font-mono text-xl uppercase tracking-widest"
                value={code}
        ...
        {step === 2 && roomInfo && (
          <div className="py-4 space-y-5">
            <div className="flex flex-col items-center p-4 rounded-xl bg-muted/30 border border-border text-center">
              <span className="text-[10px] uppercase tracking-widest text-muted-foreground/60 font-bold mb-1">Bạn đang vào phòng</span>
              <h3 className="text-lg font-bold text-foreground">{roomInfo.name}</h3>
              <p className="text-xs text-muted-foreground mt-1">Chủ phòng: {roomInfo.host_name}</p>
              <div className="flex items-center gap-2 mt-3 px-3 py-1 rounded-full bg-muted/40 border border-border text-[10px] text-muted-foreground font-bold">
            ...
            <div className="p-4 rounded-xl bg-yellow-500/10 border border-yellow-500/20 space-y-3">
              ...
              <p className="text-xs text-foreground/70 leading-relaxed">
                ...
                <span className="font-bold text-foreground">• Toàn bộ tài liệu bạn upload</span> trong phòng này sẽ được chia sẻ với các thành viên khác. <br />
                <span className="font-bold text-foreground">• Bạn có thể xem và hỏi AI</span> dựa trên tài liệu của những thành viên khác trong phòng này.
              </p>
            </div>
          </div>
        )}
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          ...
          {step === 2 && (
            <>
              <Button variant="ghost" className="flex-1 h-12 rounded-xl text-muted-foreground" onClick={() => setStep(1)}>Quay lại</Button>
              <Button
